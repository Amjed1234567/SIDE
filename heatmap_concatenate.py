...
# Combine drugs and side-effects into one set of points
combined_latents = torch.cat([w, v], dim=0).numpy()
n_drugs = w.shape[0]
n_ses = v.shape[0]

# Single Ward Clustering 
linkage_matrix = linkage(combined_latents, method="ward", metric="euclidean")

# From https://docs.scipy.org/doc/scipy/reference/generated/scipy.cluster.hierarchy.dendrogram.html
# Get the leaf order from the dendrogram (indices into the combined array)
dend = dendrogram(linkage_matrix, no_plot=True)
combined_order = dend["leaves"]

# Split the combined order back into drug and side-effect indices
# Indices < n_drugs correspond to drugs
# Indices >= n_drugs correspond to side-effects (original index = combined_index - n_drugs)
drug_order = [i for i in combined_order if i < n_drugs]
se_order = [i - n_drugs for i in combined_order if i >= n_drugs]

# Reorder the Adjacency Matrix 
# Using np.ix_ to reorder rows (drugs) and columns (side-effects) simultaneously
...
plt.tight_layout()
plt.savefig("heatmap_global.png", dpi=300, bbox_inches="tight")
print(f"Ordered adjacency matrix (global hierarchy) saved as 'heatmap_global.png'.")